...
export default function WhatsAppConsole({ user }: WhatsAppConsoleProps) {
  // Config state
  ...
  // Client-side cache for recent updates
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Load both configs & requests concurrently
      const [configRes, requestsRes] = await Promise.all([
        fetch('/api/committee/whatsapp-config'),
        fetch('/api/committee/whatsapp-requests')
      ]);
      ...
      if (configRes.ok) {
        ...
        if (recentUpdate && (now - recentUpdate.timestamp) < cacheValidFor) {
          ...
          setNewWhatsAppLink(configResult.config.whatsapp_link);
          if (recentUpdate && (now - recentUpdate.timestamp) >= cacheValidFor) {
  ...
  const handleUpdateConfig = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/committee/whatsapp-config', {
        ...
        body: JSON.stringify({ whatsapp_link: newWhatsAppLink })
      });
      ...
      if (response.ok) {
        if (result.success && result.message) {
          const updatedConfig = { whatsapp_link: newWhatsAppLink };
  ...
  const exportAccessLogs = () => {
    ...
    const csvContent = [
      ...
      ...data.accessLogs.map(log => [
        log.email, log.phone || 'N/A', log.verification_method, log.status,
  ...
  if (loading && !data) {
    return (
      <div className="flex flex-col items-center justify-center p-12">
        ...
        <span className="mt-4 text-slate-grey font-sans font-medium">Loading console...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Manual Join Requests Section (Prominent) */}
                    ...
                    {processedRequests.map((req) => (
                      <tr key={req.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-5 py-3 text-slate-grey max-w-[200px]">
                          ...
                          {req.trips && (
                            <div className="text-xs text-gray-500 mt-1 truncate" title={req.trips}>
                              {req.trips}
                            </div>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-3 text-xs text-slate-grey">
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div className="mb-4 space-y-2">
              ...
              <p className="text-xs text-slate-grey mt-2">
                Used to redirect members after they verify their identity.
              </p>
            </div>
              ...
              <table className="w-full text-sm font-sans relative">
                ...
                <tbody className="divide-y divide-gray-100">
                  {data?.accessLogs?.length ? (
                    ...
                    <tr><td colSpan={2} className="px-3 py-6 text-center text-gray-500 text-xs">No access logs found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
