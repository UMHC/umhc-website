...
export default function WomxnVerificationForm() {
  ...
  useEffect(() => {
    if (turnstileLoaded && window.turnstile && !turnstileWidgetId) {
      const container = document.getElementById('turnstile-container-womxn');
      if (!container) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const websiteField = document.querySelector('input[name="website-womxn"]') as HTMLInputElement;
      const websiteValue = websiteField?.value || '';

      const response = await fetch('/api/womxn-whatsapp-simplified', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, phone, turnstileToken, website: websiteValue }),
      });
      ...
      if (response.ok) {
        setError('');
        setSuccess('Success! A verification link has been sent to your university email address. Check your inbox and click the link to join the Womxn WhatsApp group.');
        setEmail('');
        setPhone('');
        setTurnstileToken('');
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8 items-center justify-start w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl mx-auto">
      ...
      <div className="flex flex-col items-center justify-start w-full">
        ...
        <div className="text-center w-full">
          <h1 className="font-sans font-semibold text-3xl sm:text-4xl md:text-5xl text-deep-black mb-3 sm:mb-4 leading-tight">
            Join Our Womxn Community
          </h1>
          <p className="font-sans font-medium text-base sm:text-lg text-deep-black px-2">
            Enter your university email address to get instant access to the UMHC Womxn WhatsApp group.
            If you don&apos;t have a <code className="font-mono bg-gray-100 px-1 rounded">.ac.uk</code> email,
            you can{' '}
            <a href="/womxn-whatsapp-request" className="font-medium text-umhc-green hover:underline">
      ...
      <form
        ...
        role="form"
        aria-label="Womxn WhatsApp verification form"
      >
        {/* University Email Input */}
        <div className="flex flex-col gap-1 w-full">
          <label htmlFor="womxn-email" className="font-sans font-medium text-sm text-deep-black">
          ...
          <input
            ...
            aria-describedby="womxn-email-help"
          />
          <p id="womxn-email-help" className="text-xs sm:text-sm text-slate-grey mt-1 font-sans">
        ...
        {/* Phone Number Input */}
        <div className="flex flex-col gap-1 w-full">
          <label htmlFor="womxn-phone" className="font-sans font-medium text-sm text-deep-black">
            Phone Number
          </label>
          <input
            type="tel"
            ...
            required
            className="w-full h-12 sm:h-14 md:h-15 px-3 sm:px-4 py-3 sm:py-4 bg-cream-white border-2 border-gray-200 rounded-lg focus:border-umhc-green focus:outline-none transition-colors font-sans text-sm sm:text-base"
            aria-describedby="womxn-phone-help"
          />
          <p id="womxn-phone-help" className="text-xs sm:text-sm text-slate-grey mt-1 font-sans">
        ...
        <div className="flex flex-col gap-2 w-full">
          <div className="flex items-start gap-3">
            ...
            />
            <label htmlFor="womxn-terms" className="font-sans text-sm text-deep-black leading-relaxed">
              I agree to the{' '}
              <a
                href="/terms"
                target="_blank"
        ...
        <div className="flex flex-col items-center gap-2">
          <label htmlFor="turnstile-container-womxn" className="sr-only">
            Security verification challenge
          </label>
          <div
            id="turnstile-container-womxn"
          ...
          >
            {error.includes('manual request') ? (
              <>
                ...
                <a href="/womxn-whatsapp-request" className="text-umhc-green underline hover:text-stealth-green">
                  request access manually
                </a>
        ...
        <div className="flex justify-center">
          <Button
            ...
            aria-describedby="womxn-submit-help"
          >
            {isSubmitting ? 'Sending...' : 'Get Access Link'}
          </Button>
          <div id="womxn-submit-help" className="sr-only">
            {!canSubmit && !turnstileToken
              ? 'Complete security verification to enable button'
              : 'Submit the form to receive Womxn WhatsApp access link via email'}
          </div>
        </div>
      </form>
    </div>
